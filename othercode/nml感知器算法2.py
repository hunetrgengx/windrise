...
class TrainDataLoader: #构造数据
    def __init__(self):
        pass
    def GenerateRandomData(self, count, gradient, offset): #输入count，gradiend offset作为入参
        x1 = np.linspace(1, 5, count)  #定义x1为1到5，平均分count个的数组
        x2 = gradient*x1 + np.random.randint(-10,10,*x1.shape)+offset #利用x1的格式，创造随机数列x2
        dataset = []
        y = []
        for i in range(*x1.shape):
            dataset.append([x1[i], x2[i]]) #dateset序列，依次添加x1，x2
