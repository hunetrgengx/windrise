...
cursor = conn.cursor()
cursor.execute("SELECT cointype as 币种, to_char( create_time, 'yyyy-MM-dd' ) as 充币时间, COUNT ( ID ) AS 充币笔数, count(distinct uid), SUM ( amount ) AS 充币金额 FROM withdrawcharge_op T  WHERE T.op_type = 0  AND create_time >= '" + lastweekdate+ "'  GROUP BY cointype, to_char( create_time, 'yyyy-MM-dd' )  ORDER BY to_char( create_time, 'yyyy-MM-dd' ),cointype")
 ## 获取SELECT返回的元组
rows = cursor.fetchall()
...
cursor.execute("select (select sum(lock_bz) from  eos_lock_detail  where to_char(create_time, 'YYYY-MM-DD')<=' " +str(date)+ " ')-(select sum(unlock_bz) from  eos_unlock_detail  where to_char(create_time, 'YYYY-MM-DD')<=' " + str(date) + " ')")
rows = cursor.fetchall()
for i in range(a+3,len(rows)+a+3):
         for j in range(len(rows[i-a-3])):
              sheet9.write(i+1,j,rows[i-a-3][j])
...
cursor.execute("select nonfee_count from eos_period_cfg    where to_char(effect_time, 'YYYY-MM-DD')<='" +str(date)+ "'  order BY effect_time limit 1")
rows = cursor.fetchall()
for i in range(a+3,len(rows)+a+3):
         for j in range(len(rows[i-a-3])):
              sheet9.write(i+1,j,rows[i-a-3][j])
...
cursor.execute("select  distinct to_char(create_time, 'YYYY-MM-DD') date,difficulty from eos_nonfee_info    where to_char(create_time, 'YYYY-MM-DD HH24:mi:ss') like '%00:00:00' order by date desc")
rows = cursor.fetchall()
for i in range(a+3,len(rows)+a+3):
         for j in range(len(rows[i-a-3])):
              sheet9.write(i+1,j,rows[i-a-3][j])
...
cursor.execute("select to_char(create_time, 'YYYY-MM-DD') date,count(*)from  eos_resource_detail  group by to_char(create_time, 'YYYY-MM-DD') order by date desc")
rows = cursor.fetchall()
for i in range(a+3,len(rows)+a+3):
         for j in range(len(rows[i-a-3])):
              sheet9.write(i+1,j,rows[i-a-3][j])  
